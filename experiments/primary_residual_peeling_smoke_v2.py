...
CORRECTED_MAP_COLUMNS = [
    ...
    "representative_label_verified",
    "representative_fit_label",
    "edge_cochain_solve_status",
    "representative_selection_status",
    "original_map",
    "correction_map",
        ...
        rows.append(
            {
                ...
                "representative_label_verified": choice.representative_label_verified if choice else np.nan,
                "representative_fit_label": choice.representative_fit_label if choice else np.nan,
...
def cumulative_update_allowed(result, quotient_reduces: bool, permutation_safe: bool) -> bool:
    return bool(getattr(result, "implemented", False) and quotient_reduces and permutation_safe)
...
def evaluate_prime_family(
    ...
    for peel in plan:
        ...
        if cumulative_update_allowed(cumulative_result, cumulative_q_reduces, cumulative_perm_safe):
        ...
        cumulative_metrics = evaluate_c2m3_from_pairwise(setting, bundle, cumulative_result.corrected) if cumulative_result.implemented else None
...
def write_report(args, settings: list[SelectedSetting], rows: pd.DataFrame, stats: pd.DataFrame, maps: pd.DataFrame) -> None:
    ...
    verification = maps.get("representative_label_verified", pd.Series(dtype=object))
    verified_true = int(verification.astype(str).str.lower().eq("true").sum()) if len(verification) else 0
    verified_false = int(verification.astype(str).str.lower().eq("false").sum()) if len(verification) else 0
    verified_missing = int(verification.isna().sum()) if len(verification) else 0
...
- Representative label verification: `verified={verified_true}, failed={verified_false}, not_directly_assigned={verified_missing}`
...
def main(argv: list[str] | None = None) -> int:
    ...
    exact_solve_count = int(rows["edge_cochain_solve_status"].astype(str).str.contains("exact_quotient_cochain_solve", regex=False).sum())
    representative_available_count = int(rows["representative_selection_status"].astype(str).eq("representative_correction_available").sum())
    verification = maps_out.get("representative_label_verified", pd.Series(dtype=object))
    verified_true = int(verification.astype(str).str.lower().eq("true").sum()) if len(verification) else 0
    verified_false = int(verification.astype(str).str.lower().eq("false").sum()) if len(verification) else 0
    verified_missing = int(verification.isna().sum()) if len(verification) else 0
    ...
    print(f"Representative label verification: verified={verified_true}, failed={verified_false}, not_directly_assigned={verified_missing}")
    ...
    print(f"Best peeled validation delta vs baseline: {float(best_peeled.max()) if len(best_peeled) else 'not_run'}")
    print(f"Best cumulative validation delta vs baseline: {float(best_cum.max()) if len(best_cum) else 'not_run'}")
    print("Best monomial validation delta vs baseline:")
