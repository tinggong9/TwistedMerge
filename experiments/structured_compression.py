...
def main() -> None:
    groups = {"S3": symmetric_group_3(), "D4": dihedral_group(4)}; rows = []
    for seed in range(5):
    ...
    claims.extend([{"group": "FashionMNIST", "claim": "teacher_eligible", "value": chart_gate.exists() and "confirmation_passed,True" in chart_gate.read_text()}, {"group": "ModelNet10", "claim": "teacher_eligible", "value": multiview_gate.exists() and "complete_multiview_gate_passed,True" in multiview_gate.read_text()}])
    ...
    (DEST / "compression_report.md").write_text(
        ...
        f"{passed} of two controlled teachers passed the 95%-gain and 25%-measured-reduction gate. Fashion-MNIST and "
        "ModelNet10 teachers are included only when their upstream confirmation gates pass.\n",
        encoding="utf-8",
    )
