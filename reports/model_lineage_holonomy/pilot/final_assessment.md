...
Mode: **pilot**. Gate status: `{'H1': False, 'H2': False, 'H3': False, 'H4': False}`.
...
8. **Which layers carried the strongest stable signal?** No layer passed the stability gate; `adapter` had the largest raw distance (0.9731) but is not a stable signal.
9. **What is the strongest main-paper claim?** No main-paper claim is available from the three-seed pilot; complete the frozen two-seed extension without broadening the design.
10. **Should holonomy application experiments stop?** No. Complete seeds 3-4 as preregistered, but do not add another dataset, corruption, backbone, predictor, or cycle estimator.
