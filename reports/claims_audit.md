...
| Cycle obstruction score predicts weight-average merge degradation beyond the trivial number-of-models confound. | Not yet supported | `reports/fixed_setting_verification_report.md` and `reports/fixed_setting_full_run_interpretation.md` now record a quality-gated `mlp2` run on MNIST and Fashion-MNIST with 30 observed seeds per fixed setting. Individual-model accuracy passes the gate, but all observed settings remain unsupported because Pearson/Spearman signs are mixed or the bootstrap Pearson lower bound is nonpositive. |
...
| real_fixed_setting_obstruction_prediction | not supported by the quality-gated full run | experiments/model_merging_fixed_setting_verification.py; reports/fixed_setting_verification_report.md; reports/fixed_setting_full_run_interpretation.md; reports/csv/fixed_setting_verification_stats.csv | The fixed-setting script is the paper-grade real verification entry point. The current quality-gated MNIST/Fashion-MNIST run is negative-but-useful: local model quality and repeated-seed counts pass, but no observed setting passes the predefined obstruction-correlation gate. | Do not claim raw weight-average degradation prediction, broad real-model prediction, or positive empirical support unless a future full observed repeated-seed run passes the gate. |
