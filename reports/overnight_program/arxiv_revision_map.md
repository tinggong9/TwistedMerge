# Immediate manuscript revision map
...
- Replace any aggregated practical-selector numbers with the fresh Stage 1 table. The selector delta versus ordinary greedy soup is -0.001398, 95% CI [-0.002066, -0.000731].
- Replace any global provenance commit in the evidence manifest with each artifact's `actual_execution_commit`.
- Replace the old context-independent two-loop accuracy discussion with the new controlled context-dependent result, decision A, while retaining the old structural construction as a separate result.
